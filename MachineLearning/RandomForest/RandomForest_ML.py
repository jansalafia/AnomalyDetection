...
def run_best_model(path: str = "CSVs/newDataset.csv",
    ...
    summary_csv = os.path.join(output_dir, "randomforest_summary.csv")
    ...
    # Handle max_features fix
    max_features_val = p.get("max_features", "sqrt")
    if max_features_val == "auto":
    ...
    pipe = Pipeline([
        ...
        ("clf", RandomForestClassifier(
            ...
            max_depth=None if pd.isna(p.get("max_depth")) else int(p.get("max_depth")),
            min_samples_split=int(p.get("min_samples_split", 2)),
            min_samples_leaf=int(p.get("min_samples_leaf", 1)),
            max_features=max_features_val,
            class_weight=None if p.get("class_weight") == "None" else p.get("class_weight"),
